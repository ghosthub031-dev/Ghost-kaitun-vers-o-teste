...
function hoangtuveu()
    ...
    -- ============================================================
    -- UI: ProxyLib shell (Ghost Kaitun BETA) - replaces the old
    -- Dynamic Island panel. SetText now feeds ProxyLib paragraphs;
    -- every farm-logic call site below stays untouched.
    -- ============================================================
    ...
    local Window   = ProxyLib.new()

    local MainWindow = Window:CreateWindow({
        Title    = "Ghost Kaitun BETA",
        Subtitle = "@ghost.scripts.01",
        Icon     = "rbxassetid://139984424145579",
        ...
        BackgroundImage = { Id = "rbxassetid://000000000", Active = false },
    ...
    local Tabs = {
        Kaitun = MainWindow:CreateTab({ Title = "Kaitun BETA", Subtitle = "@ghost.scripts.01", Icon = "rbxassetid://139984424145579" }),
        Status = MainWindow:CreateTab({ Title = "Configurações", Subtitle = "@ghost.scripts.01", Icon = "rbxassetid://139984424145579" }),
    }

    Tabs.Kaitun:Select()

    Tabs.Kaitun:CreateSection({ Text = "Kaitun Status", Icon = "rbxassetid://0" })

    local MainTask = Tabs.Kaitun:CreateParagraph({
        Title       = "Main Farm Status",
        Description = "",
    })

    local SubTask = Tabs.Kaitun:CreateParagraph({
        Title       = "Sub Farm Status",
        Description = "",
    })

    local TimeKaitun = Tabs.Kaitun:CreateParagraph({
        Title       = "Kaitun Run Time",
        Description = "",
    })

    -- Diagnostics paragraph: original SetText keys that have no dedicated
    -- paragraph (Currencies, Melees, DebugLine) land here so SetText never
    -- writes to a nil target. [CONFIG TAB] movido para a aba Kaitun — a
    -- 2ª aba agora é 100% dedicada às configurações do script.
    local DiagTask = Tabs.Kaitun:CreateParagraph({
        Title       = "Diagnostics",
        Description = "",
    })

    -- [CONFIG TAB] Os parágrafos de status da 2ª aba foram removidos.
    -- Os loops do rodapé que escreviam neles continuam rodando, mas agora
    -- escrevem em stubs no-op (nada aparece na tela, nada quebra).
    local function NewStatusParagraph(_title)
        return { SetDescription = function() end, Destroy = function() end }
    end
    local StatLevel = NewStatusParagraph("Level")
    local StatRace = NewStatusParagraph("Race")
    local StatBeli = NewStatusParagraph("Beli")
    local StatFrag = NewStatusParagraph("Fragments")
    local StatGodHuman = NewStatusParagraph("God Human")
    local StatCDK = NewStatusParagraph("Cursed Dual Katana")
    local StatSkullGuitar = NewStatusParagraph("Skull Guitar")
    local StatMirrorFractal = NewStatusParagraph("Mirror Fractal")
    local StatValkyrie = NewStatusParagraph("Valkyrie Helm")
    local StatPullLever = NewStatusParagraph("Pull Lever")

    -- Shared refs for the bottom status loops (they run in their own
    -- task.spawn and cannot see these locals).
    getgenv().Ghost_StatusRefs = {
        Level = StatLevel,
        Race = StatRace,
        Beli = StatBeli,
        Frag = StatFrag,
        GodHuman = StatGodHuman,
        CDK = StatCDK,
        SkullGuitar = StatSkullGuitar,
        MirrorFractal = StatMirrorFractal,
        Valkyrie = StatValkyrie,
        PullLever = StatPullLever,
    }

    -- ==============================================================================
    -- [CONFIG TAB] 2ª ABA = CONFIGURAÇÕES DO SCRIPT (muda ao vivo, sem restart)
    -- Cada controle grava na MESMA variável que a lógica do farm lê
    -- (Config.* e getgenv().Ghost_FastAttackPower) — nada de variável inventada.
    -- ==============================================================================
    local Power = getgenv().Ghost_FastAttackPower or {PerTick = 5, TickRate = 0.01, ClickRate = 0.005}
    getgenv().Ghost_FastAttackPower = Power

    local function CfgToggle(title, desc, saveId, getter, setter)
        Tabs.Status:CreateToggle({
            Title       = title,
            Description = desc,
            Default     = getter() and true or false,
            SaveId      = saveId,
            Callback    = function(Value) pcall(setter, Value) end
        })
    end

    local function PowerPreset(label, perTick, tickRate, clickRate)
        Tabs.Status:CreateButton({
            Title    = label,
            Callback = function()
                Power.PerTick   = perTick
                Power.TickRate  = tickRate
                Power.ClickRate = clickRate
                getgenv().Ghost_Notify("Configurações", "Potência aplicada: " .. label)
            end
        })
    end

    Tabs.Status:CreateSection({ Text = "⚡ Potência de Ataque", Icon = "rbxassetid://0" })
    PowerPreset("Potência: MÁXIMA (~1000 golpes/s)", 10, 0.01, 0.002)
    PowerPreset("Potência: ALTA (padrão, ~500 golpes/s)", 5, 0.01, 0.005)
    PowerPreset("Potência: SEGURA (anti-lag)", 2, 0.02, 0.02)
    CfgToggle("Auto Click", "Clique físico junto com o Fast Attack", "cfg_autoclick",
        function() return (Power.ClickRate or 0) > 0 end,
        function(v) Power.ClickRate = v and 0.005 or 0 end)

    Tabs.Status:CreateSection({ Text = "🥋 Itens & Missões", Icon = "rbxassetid://0" })
    local function ItemToggle(key, title, desc, saveId)
        CfgToggle(title, desc, saveId,
            function() return Config.Items[key] ~= false end,
            function(v) Config.Items[key] = v end)
    end
    ItemToggle("AutoFullyMelees",  "Auto Full Melees",      "Compra e farma todos os melees",     "cfg_fulmelees")
    ItemToggle("Saber",            "Saber",                 "Farm da espada Saber",               "cfg_saber")
    ItemToggle("CursedDualKatana", "Cursed Dual Katana",    "Farm da CDK",                        "cfg_cdk")
    ItemToggle("SoulGuitar",       "Soul Guitar",           "Farm da Soul Guitar",                "cfg_soulguitar")
    ItemToggle("RaceV2",           "Race V2",               "Evoluir a raça para V2",             "cfg_racev2")
    ItemToggle("AutoRaceV3",       "Race V3",               "Evoluir a raça para V3",             "cfg_racev3")
    ItemToggle("AutoRandomFruit",  "Auto Random Fruit",     "Trocar frutas aleatoriamente",       "cfg_randomfruit")
    CfgToggle("Auto Ken (Observation)", "Observation Haki automático", "cfg_ken",
        function() return Config.AutoKen ~= false end,
        function(v) Config.AutoKen = v end)
    CfgToggle("Bring Mob (base)", "Motor de puxar mobs do farm (usado pelo Bring Mob da missão)", "cfg_bringmobs",
        function() return Config.BringMobs ~= false end,
        function(v) Config.BringMobs = v; getgenv().Ghost_BringQuestMobs = v end)
    CfgToggle("Auto Comprar Melee", "Compra o melee ao atingir a mastery necessária", "cfg_meleebuy",
        function() return Config.Melee.AutoBuy ~= false end,
        function(v) Config.Melee.AutoBuy = v end)

    Tabs.Status:CreateSection({ Text = "⚔️ Espadas (farm de espada)", Icon = "rbxassetid://0" })
    for _, swName in ipairs({"Shark Saw", "Wardens Sword", "Pole (1st Form)", "Gravity Blade", "Longsword", "Rengoku", "Flail", "Twin Hooks"}) do
        local name = swName
        local sid = "cfg_sw_" .. tostring(name):gsub("%W", "")
        CfgToggle(name, "Farm da espada: " .. name, sid,
            function() return Config.Sword[name] ~= false end,
            function(v) Config.Sword[name] = v end)
    end

    Tabs.Status:CreateSection({ Text = "👹 Armas de Boss", Icon = "rbxassetid://0" })
    for _, bossName in ipairs({"Awakened Ice Admiral", "Tide Keeper", "Deandre", "Urban", "Diablo", "Soul Reaper", "Cake Prince", "Core", "Darkbeard", "Katakuri", "Beautiful Pirates"}) do
        local name = bossName
        local sid = "cfg_boss_" .. tostring(name):gsub("%W", "")
        CfgToggle(name, "Farm de arma do boss: " .. name, sid,
            function() return Config.BossWeapons[name] ~= false end,
            function(v) Config.BossWeapons[name] = v end)
    end

    Tabs.Status:CreateSection({ Text = "⚙️ Geral", Icon = "rbxassetid://0" })
    CfgToggle("Hop When Idle", "Troca de servidor se ficar parado 5 min", "cfg_hopidle",
        function() return Config.Configuration.HopWhenIdle ~= false end,
        function(v) Config.Configuration.HopWhenIdle = v end)
    CfgToggle("Auto Hop", "Troca de servidor automaticamente por tempo", "cfg_autohop",
        function() return Config.Configuration.AutoHop ~= false end,
        function(v) Config.Configuration.AutoHop = v end)
    CfgToggle("FPS Boost", "Otimização gráfica para ganhar FPS", "cfg_fpsboost",
        function() return Config.Configuration.FpsBoost == true end,
        function(v) Config.Configuration.FpsBoost = v end)
    CfgToggle("Black Screen", "Tela preta para economizar recursos", "cfg_blackscreen",
        function() return Config.Configuration.blackscreen == true end,
        function(v) Config.Configuration.blackscreen = v end)
    CfgToggle("Low Graphics", "Gráficos no mínimo", "cfg_lowgraphics",
        function() return Config.Configuration.LowGraphics ~= false end,
        function(v) Config.Configuration.LowGraphics = v end)

    -- ==============================================================================
    -- BRIDGE (communicates via getgenv, no need to know ProxyLib)
    -- ==============================================================================
    getgenv().Ghost_Notify = function(title, text)
        pcall(function()
            MainWindow:Notify({
                TitleWords = { tostring(title or "Kaitun BETA") },
                TextWords  = { tostring(text or "") },
                Icon = "rbxassetid://139984424145579",
                Duration   = 5,
            })
        end)
    ...
    getgenv().Ghost_SetMainTask = function(text)
        pcall(function() MainTask:SetDescription(tostring(text or "")) end)
    end

    getgenv().Ghost_SetSubTask = function(text)
        pcall(function() SubTask:SetDescription(tostring(text or "")) end)
    end

    getgenv().Ghost_SetKaitunTime = function(text)
        pcall(function() TimeKaitun:SetDescription(tostring(text or "")) end)
    end

    -- ==============================================================================
    -- TOGGLE ON / OFF KAITUN
    -- ==============================================================================
    Tabs.Kaitun:CreateToggle({
        Title       = "Auto Kaitun",
        Description = "Enable/Disable Kaitun BETA",
        Default     = true,
        SaveId      = "auto_kaitun",
        Callback    = function(Value)
            getgenv().AutoKaitun = Value
            if Value then
                getgenv().Ghost_Notify("Kaitun BETA", "Auto Kaitun Enabled")
            else
                getgenv().Ghost_Notify("Kaitun BETA", "Auto Kaitun Disabled")
            end
        end
    })

    -- ==============================================================================
    -- BRING MOB (MISSÃO) — puxa os mobs da quest até o player
    -- ==============================================================================
    Tabs.Kaitun:CreateToggle({
        Title       = "Bring Mob (Missão)",
        Description = "Puxa os mobs da missão até você",
        Default     = true,
        SaveId      = "bring_quest_mobs",
        Callback    = function(Value)
            getgenv().Ghost_BringQuestMobs = Value
            if Value then
                getgenv().BringMonster = true
                getgenv().Ghost_Notify("Bring Mob", "Puxando os mobs da missão!")
            else
                getgenv().BringMonster = false
                getgenv().Ghost_Notify("Bring Mob", "Bring Mob desativado")
            end
        end
    })

    -- ==============================================================================
    -- YOUTUBE BUTTON (with YouTube icon)
    -- ==============================================================================
    Tabs.Kaitun:CreateButton({
        Title       = "YouTube Channel",
        Description = "@ghost.scripts.01",
        Icon        = "rbxassetid://1275974023",
        Callback    = function()
            local link = "https://youtube.com/@ghost.scripts.01?si=_UxDykEQBGg7IS70"
            if syn then
                syn.request({Url = link, Method = "GET"})
            elseif fluxus then
                fluxus.request({Url = link, Method = "GET"})
            else
                setclipboard(link)
                getgenv().Ghost_Notify("YouTube", "Link copied to clipboard!")
            end
        end
    })

    -- ==============================================================================
    -- SetText compatibility layer: same signature and keys as the old
    -- Dynamic Island panel. SetTask (farm logic) calls this 172x with
    -- keys Task1/Task2; LiveTime/Currencies/Melees/DebugLine also routed.
    -- ==============================================================================
    local SetTaskMap = {
        Task1 = MainTask,
        Task2 = SubTask,
        MainTextLabel = MainTask,
        LiveTime = TimeKaitun,
        DebugLine = DiagTask,
        Currencies = DiagTask,
        Melees = DiagTask,
    ...
    function SetText(key, text)
        task.spawn(function()
            local paragraph = SetTaskMap[key] or DiagTask
            if not paragraph then return end
            pcall(function() paragraph:SetDescription(tostring(text or "")) end)
        end)
    end
...
function RefreshPlayerData()
    ...
    -- ============================================================
    -- [TURBO] FAST ATTACK + AUTO CLICK — ajuste os 3 valores abaixo:
    --   FAST_ATTACK_FIRE_PER_TICK : golpes disparados por tick (padrão 5)
    --   FAST_ATTACK_TICK_RATE     : intervalo entre ticks em s (padrão 0.01 ≈ 100 ticks/s)
    --   AUTO_CLICK_RATE           : intervalo do clique físico em s (padrão 0.005 ≈ 200 cliques/s; 0 = desliga)
    -- Pico teórico ≈ 5 x 100 = 500 golpes/s. Se der lag/kick, abaixe
    -- FAST_ATTACK_FIRE_PER_TICK para 2-3.
    -- [TOGGLE] ambos os loops param quando o botão Auto Kaitun está OFF
    -- (nil/true = ligado; só o false explícito pausa), e tudo dentro de
    -- pcall para um loop rápido não error-spammar e travar o client.
    -- ============================================================
    -- [CONFIG TAB] A potência agora é controlada ao vivo pela 2ª aba
    -- ("Configurações" → Potência de Ataque), via getgenv().Ghost_FastAttackPower.
    local Power = getgenv().Ghost_FastAttackPower or {PerTick = 5, TickRate = 0.01, ClickRate = 0.005}
    getgenv().Ghost_FastAttackPower = Power
    local _vim = game:GetService("VirtualInputManager")
    task.spawn(function()
        while true do
            if getgenv().AutoKaitun == false then
                task.wait(0.2)
            elseif _G.FastAttack and os.time() - _G.FastAttack <= 1 then
                for _ = 1, (Power.PerTick or 5) do
                    pcall(function() h:Attack() end)
                end
                task.wait(Power.TickRate or 0.01)
            else
                task.wait(0.05)
            end
        end
    ...
    task.spawn(function()
        while true do
            if getgenv().AutoKaitun == false or (Power.ClickRate or 0) <= 0 then
                task.wait(0.2)
            elseif _G.FastAttack and os.time() - _G.FastAttack <= 1 then
                pcall(function()
                ...
                task.wait(Power.ClickRate or 0)
            else
                task.wait(0.05)
            end
        end
...
function W.Attack(target) pcall(function() _G.FastAttack = os.time() end) end
    ...
    -- [BRING MOB] flag do novo toggle "Bring Mob (Missão)" — default LIGADO
    getgenv().Ghost_BringQuestMobs = getgenv().Ghost_BringQuestMobs ~= false
    ...
    -- ============================================================
    -- [NEW] BRING MOB DA MISSÃO — puxa os mobs da quest até o player.
    -- Reaproveita o sistema BringEnemy que JÁ EXISTE (LockMob + BodyVelocity,
    -- anti-fling, SimulationRadius) alimentando PosMon/Mon automaticamente
    -- com o alvo que o dispatcher do farm escolheu (getgenv().Ghost_QuestMob,
    -- capturado dentro do CombatController.Attack) — nenhum nome de mob
    -- hardcoded: seja LevelFarm, CakePrinceTask ou BossesTask, o mob da
    -- missão em andamento é o que será puxado.
    -- Toggle: UI "Bring Mob (Missão)" → getgenv().Ghost_BringQuestMobs.
    -- Loop único (nada de duplicação se religar), para limpo quando OFF,
    -- tudo em pcall. Altura: 6 studs abaixo do player — mobs colados no pé,
    -- dentro do alcance do Fast Attack e sem chance de fling.
    -- ============================================================
    task.spawn(function()
        while task.wait(0.1) do
            pcall(function()
                if not getgenv().Ghost_BringQuestMobs then return end
                local mobName = getgenv().Ghost_QuestMob
                ...
                getgenv().BringMonster = true
                Mon = mobName
                PosMon = CFrame.new(root.Position - Vector3.new(0, 6, 0))
            end)
        end
    end)
    ...
    function CombatController.Attack(h, X, w, D)
        ...
        for y, L in (h) do
            ...
            if MonResult then
                LastFound = os.time()
                getgenv().Ghost_QuestMob = MonResult.Name -- [BRING MOB] guarda o alvo atual da missão (sem hardcode)
                local LastNoSpinCF = nil -- [NO SPIN] último ponto estável usado neste ataque
                local h, w = 0, os.time()
                ...
                while task.wait() do
                    ...
                    -- [NO SPIN] Posição FIXA acima do mob — sem órbita/giro.
                    -- Antes: CaculateCircreDirection girava 20° a cada 0.01s
                    -- num raio de 40 studs ao redor do mob (chamada TODA
                    -- iteração do loop) — o personagem ficava rodando em
                    -- volta do alvo sem parar. Agora: um único ponto estável
                    -- (35 studs acima do mob, dentro do alcance 150 do
                    -- check de ataque e do Fast Attack de 65), e o tween só
                    -- é recriado se o mob andar mais de 10 studs — zero
                    -- reorientação por tick, farm parado e direto no alvo.
                    local noSpinCF = CFrame.new(p.Position + Vector3.new(0, 35, 0))
                    if not LastNoSpinCF or (LastNoSpinCF.Position - noSpinCF.Position).Magnitude > 10 then
                        LastNoSpinCF = noSpinCF
                        TweenController.Create(noSpinCF)
                    end
    ...
    ParsingTimes = 0
    -- [FIX TOGGLE] O botão "Auto Kaitun" só gravava getgenv().AutoKaitun
    -- (nenhum código lia essa variável), então ativar/desativar não fazia
    -- NADA — o dispatcher continuava farmando sempre. Agora o dispatcher
    -- respeita o estado do toggle de verdade.
    if getgenv().AutoKaitun == nil then
        getgenv().AutoKaitun = true
    end
    function RefreshTasksData()
        if not getgenv().AutoKaitun then
            SetTask("MainTask", "Auto Kaitun: OFF — pausado pelo toggle")
            SetTask("SubTask", "Idle")
            return
        end
        if _G.Stop then return end
...
--============================================================
-- STATUS UI (ProxyLib) - same loops as the old Noguchi panel,
-- now feeding the Status tab paragraphs instead of raw labels.
--============================================================
task.spawn(function()
    local Players = game:GetService("Players")
    ...
    -- God Human unlock poll (original logic, kept verbatim)
    ...
    -- HexUI compatibility stub (same public surface as the old panel)
    getgenv().HexUI = {
        addItemLabel = function() return { Destroy = function() end } end,
        getGodHuman = function() return godhumanUnlocked end,
    }

    local Refs = getgenv().Ghost_StatusRefs
    if type(Refs) ~= "table" then return end

    -- stats (1s) - original loop, paragraph-backed
    task.spawn(function()
        while task.wait(1) do
            pcall(function()
                local data = LP:FindFirstChild("Data")
                if data then
                    Refs.Level:SetDescription("Level: " .. tostring(data:FindFirstChild("Level") and data.Level.Value or 0))
                    Refs.Race:SetDescription("Race: " .. tostring(data:FindFirstChild("Race") and data.Race.Value or "?"))
                    Refs.Beli:SetDescription("Beli: " .. tostring(data:FindFirstChild("Beli") and data.Beli.Value or 0))
                    local frags = data:FindFirstChild("Fragments")
                    Refs.Frag:SetDescription("Frag: " .. (frags and tostring(frags.Value) or "Only Sea 2, 3"))
                end
            end)
        end
    end)

    -- inventory (2s) - original loop; the notable-item checks are kept
    -- verbatim, the dynamic per-item label list has no ProxyLib
        ...
        while task.wait(2) do
            ...
            pcall(function()
                ...
                if type(inventory) == "table" then
                    local hasItem = {}
                    for _, item in pairs(inventory) do
                        hasItem[item.Name or "?"] = true
                    end
                    Refs.CDK:SetDescription(hasItem["Cursed Dual Katana"] and "\u{1F7E2} Cursed Dual Katana" or "\u{1F534} Cursed Dual Katana")
                    Refs.SkullGuitar:SetDescription(hasItem["Skull Guitar"] and "\u{1F7E2} Skull Guitar" or "\u{1F534} Skull Guitar")
                    Refs.MirrorFractal:SetDescription(hasItem["Mirror Fractal"] and "\u{1F7E2} Mirror Fractal" or "\u{1F534} Mirror Fractal")
                    Refs.Valkyrie:SetDescription(hasItem["Valkyrie Helm"] and "\u{1F7E2} Valkyrie Helm" or "\u{1F534} Valkyrie Helm")
                end
                Refs.GodHuman:SetDescription(godhumanUnlocked and "\u{1F7E2} GodHuman" or "\u{1F534} GodHuman")
                local doorOpen = CommF:InvokeServer("CheckTempleDoor")
                Refs.PullLever:SetDescription(doorOpen and "\u{1F7E2} Pull Lever" or "\u{1F534} Pull Lever")
            end)
        end
    end)
end)
